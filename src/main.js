...
import { registerFileListAction, registerFileAction as registerFileAction, Permission } from '@nextcloud/files'
...
async function handleClick(resource, batch = false) {
	// Normalize NC32 vs NC33 resource
	const file = nextcloudVersionIsGreaterThanOr32
		? resource.nodes[0] // NC 33+
		: resource.data // NC 28–32

	let resourceURI
	let shareType = file.attributes['share-types']['share-type']
	...
	if (shareType.some((value) => value === 3)) {
		resourceURI = await getResourcePublicLink(file) + '/download'
	} else {
		// There is currently no public share link -> generate direct dowload link
		resourceURI = await generateDirectDownloadLink(file)
	}

	if (useSwitchboardPopUp) {
		openInSwitchboardPopUp(resourceURI, file.mime)
	} else {
		openInSwitchboard(resourceURI, file.mime)
	}
}
...
if (nextcloudVersionIsGreaterThanOr32) {
	...
	const baseAction = {
		...
		enabled: (ctx) => {
			if (!ctx || !Array.isArray(ctx.nodes)) {
				return false
			}
			const { nodes } = ctx
			...
			// All selected nodes must be files, and none may be folders or incomplete
			if (files.length !== nodes.length) {
				return false
			}
			// All files must have READ permission
		...
		exec: async ({ nodes }) => {
			...
			for (const file of files) {
				await handleClick({ nodes: [file] }, true)
			}
			return null
		},
		order: 22,
	...
	registerFileAction({
		...baseAction,
		// row actions must NOT have "multiple"
		inline: () => false,
	})
	registerFileListAction({
		...baseAction,
		id: 'switchboardbridge-action-multi', // must be unique
		multiple: true,
	})
